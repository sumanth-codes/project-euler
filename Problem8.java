...
public class Problem8{
    static long maxProduct(String number){
    ...
    public static void main(String[] args){
        ...
        String[] numsArr = number.split("0");
        ArrayList<String> qualifiedNums = new ArrayList<>();
        long max = 1;

        for(String str : numsArr){
            if(str.length() >= 13)
            qualifiedNums.add(str);
        }

        for(String str : qualifiedNums){
            long product = maxProduct(str);
            if(max < product)
            max = product;
        }
